...
class TGLF():
    ...
    def read_sum_flux_spectrum(self,run_path=None,nspecies=None,nfields=None):
        ...
        if lines:
            ...
            # TODO bug fix for 'sum' being the same for multiple fluxes/species
    ...
    def write_input(self,path=None,file='input.tglf',header=True,verbose=False,ignore_defaults=False,overwrite=True):
        # if unspecified, for convenience check for run path in metadata
    ...
    def run(self,path=None,gacode_platform=None,gacode_root=None,init_gacode=True,verbose=False,collect=None,collect_essential=True,eigenfunctions=False):
        ...
        if verbose:
            ...
            execution.read()
        self.metadata['run_time'] = copy.deepcopy(time.time()-time0)
    ...
    def run_1d_scan(self,path=None,var=None,values=[],verbose=False,collect_essential=False,write_scan=False,return_self=True):
        ...
        if 'scan_type' not in self.metadata:
            self.metadata['scan_type'] = '1D'
        if 'start_time' not in self.metadata:
            self.metadata['start_time'] = copy.deepcopy(time.time())
        if 'scan_type' in self.metadata and self.metadata['scan_type']=='1D':
            self.metadata['num_runs'] = len(list(values))
        ...
        for value in values:
            if verbose or verbose==0:
                if 'run_time' in self.metadata:
                    PRECURSOR = ERASE_LINE + CURSOR_UP_ONE + ERASE_LINE
                    eta = time.ctime(self.metadata['start_time']+self.metadata['num_runs']*self.metadata['run_time'])
                else:
                    PRECURSOR = ERASE_LINE 
                    eta = 'calculating...'
                # print a progress %
                print('{} TGLF 1D scan {}% complete'.format(PRECURSOR,round(100*(find(value,values))/len(values))),flush=True,end='\n')
                # print estimated time of completion
                print('{} Estimated time of completion: {}'.format(ERASE_LINE,eta),flush=False,end='\r')
            ...
            if var == 'RLNS_1':
                if self.input['NS'] == 4:
                    self.input['RLNS_2'] = float(((self.input['AS_1']*self.input['RLNS_1'])-(self.input['AS_3']*self.input['ZS_3']*self.input['RLNS_3'])-(self.input['AS_4']*self.input['ZS_4']*self.input['RLNS_4']))/(self.input['AS_2']*self.input['ZS_2']))
    ...
    def run_2d_scan(self,path=None,var_y=None,values_y=[],var_x=None,values_x=[],verbose=False,collect_essential=False,write_scan=False,return_self=True):
        ...
        self.metadata['scan_type'] = '2D'
        self.metadata['start_time'] = copy.deepcopy(time.time())
        self.metadata['num_runs'] = len(list(values_x))*len(list(values_y))
        ...
        for value in values_y:
            ...
            # if ne gradient scan update ni density gradient to maintain quasineutrality
            # ((dens(1)*!dummy(1))-(dens(3)*charge(3)*omn(3))-(dens(4)*charge(4)*omn(4)))/(dens(2)*charge(2))
            if var_y == 'RLNS_1':
                if self.input['NS'] == 4:
                    self.input['RLNS_2'] = float(((self.input['AS_1']*self.input['RLNS_1'])-(self.input['AS_3']*self.input['ZS_3']*self.input['RLNS_3'])-(self.input['AS_4']*self.input['ZS_4']*self.input['RLNS_4']))/(self.input['AS_2']*self.input['ZS_2']))
                elif self.input['NS'] == 3:
    ...
    def collect_output(self,run_path=None,essential=True,eigenfunctions=False,overwrite=True,verbose=False):
        # if unspecified, for convenience check for run path in metadata
        if not run_path and 'run_path' in self.metadata:
            run_path = self.metadata['run_path']
            ...
            if file in output_run:
                if essential:
                    if output_files[file]['essential']:
                        try:
                            output_files[file]['method'](run_path=run_path)
                        except:
                            if verbose:
        ...
        if eigenfunctions:
            ...
            _collect = copy.deepcopy(self.collect)
            # switch off collect to only return the eigenfunction dict
            self.collect = False
            ...
            for ky in ky_list:
                file = 'out.tglf.wavefunction_ky={:.6f}'.format(ky)
                if 'eigenfunctions' not in self.output:
                    self.output['eigenfunctions'] = {}
                if not os.path.isfile(run_path+file) or overwrite:
                    self.input['KY'] = ky
                    self.write_input()
                    self.run()
                    ...
                    if _eigenfunctions['nmodes'] < self.metadata['nmodes'] or ('nfields' in self.metadata and _eigenfunctions['nfields'] < self.metadata['nfields']):
                        self.input['KY'] = ky
                        self.write_input()
                        self.run()
                        eigenfunction = self.read_wavefunction()['eigenfunction']
    ...
    # plotting functions
    def plot_eigenvalue_spectra(self,run_path=None,output=None,modes=[1],figure=None,label=None,align='right',show=True,save=False,file=None):
        ...
        if 'eigenvalues' in output:
            ...
            if figure:
                fig, axes = plt.subplots(2,1,sharex=True,constrained_layout=True,figsize=(4,8),num=figure)
            else:
                fig, axes = plt.subplots(2,1,sharex=True,constrained_layout=True,figsize=(4,8))
            for key_mode in modes:
                if modes[-1] > 1 and label:
                    label_ = label + ', mode {}'.format(key_mode)
                else:
                    label_ = label
                axes[0].plot(ky,gamma[key_mode],'.-',label=label_)
                axes[1].plot(ky,omega[key_mode],'.-')
            ...
            if align_y <= 0.94:
                axes[1].text(align_x,align_y+0.015,sign_labels[0],ha=align,va='bottom',fontdict=mono,transform=axes[1].transAxes)
            if align_y >= 0.05:
                axes[1].text(align_x,align_y-0.015,sign_labels[1],ha=align,va='top',fontdict=mono,transform=axes[1].transAxes)

            axes[1].set_xscale('log')
            axes[1].set_xlabel('ky')
            ...
            if save:
                if not run_path and 'run_path' in self.metadata:
                    run_path = self.metadata['run_path']
                if file:
                    fig.savefig('{}{}'.format(run_path,file),format='pdf')
                else:
    ...
    def plot_omega_spectrum(self,run_path=None,output=None,modes=[1],figure=None,label=None,align='right',show=False,save=False,file=None):
